...
function App() {
  ...
  const handleGenerateAll = async () => {
    ...
    const toProcess = imageFiles.filter(img => !img.generatedPrompt);
    
    await Promise.all(toProcess.map(img => handleGeneratePrompt(img.id)));
    
    setIsGeneratingAll(false);

    if (toProcess.length > 0) {
      alert("Đã hoàn thành tạo prompt");
    }
  };
